...
from budget_guard.adapters.limiter import LogLimiter, VolcLimiter, WebhookLimiter
...
def limiter_provider():
    if settings.limiter_provider == "log":
        return LogLimiter()
    if settings.limiter_provider == "webhook":
        return WebhookLimiter(
            settings.limiter_webhook_url, settings.limiter_webhook_token, settings.dry_run
        )
