...
def poll(billing_cycle: str | None = None):
    try: return BillingPoller(billing_provider(), limiter_provider()).run_once(billing_cycle)
...
def events(limit: int = 100):
    with SessionLocal() as db:
        ...
        return [{"project_id":r.project_id,"billing_cycle":r.billing_cycle,"state":r.state,"amount":str(r.amount),"budget":str(r.budget),"created_at":r.created_at} for r in rows]
